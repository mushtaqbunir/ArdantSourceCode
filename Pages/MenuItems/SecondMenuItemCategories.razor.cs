...
namespace ArdantOffical.Pages.MenuItems
{
    public partial class SecondMenuItemCategories
    {
        [Inject]
        ...
        async Task LoadRecords(int page = 1, int quantityPerPage = 25)
        {
            try
            {
                ...
                ListOfRecord = await UsersServices.GetFirstMenuItemNavMenuLink(paginationObj, MenuItemParentID);
        ...
        private async Task SearchChanged()
        {
            if (!string.IsNullOrEmpty(SearchKey))
            {
                ...
                try
                {
                    ListOfRecord = await UsersServices.GetFirstMenuItemNavMenuLink(paginationObj, MenuItemParentID, SearchKey);
                    if (ListOfRecord != null)
                    {
                        PerPagMenuItemRecords = ListOfRecord.MenuItemList;
                        totalPageQuantity = ListOfRecord.TotalPages;
